...
from functions.non_locan.matplotlib_custom import second_axis, second_axis_log
...
def plot_1(measure, n_points, y_1, yerr_high_1, yerr_low_1, y_2, yerr_high_2, yerr_low_2):
    ...
    if measure == "peri":
        ...
        bax.set_ylabel(r"$SA_{ch}/factor$", fontsize=21)
    elif measure == "volume":
        bax.plot(n_points, ch.calc_volume_convex_hull_3d_pt2(1, 1, n_points), color="k", lw=2)
        bax.set_ylabel(r"$V_{ch}/(\sigma_{xy}^2*\sigma_y)$", fontsize=21)
...
def plot_2(measure, sim_values, ylim, tick_spacing_y):
    """
    Generation and customization of figure 2 of "simulation_calculation_convex_hull".
    ...
    if measure == "peri":
        for i in range(1, 4, 1):
            k = 3 ** i
            ratio = sim_values[k - 3] / ch.calc_peri_convex_hull_2d_pt2(1, k)
            weight = np.ones(len(ratio)) / len(ratio)
            ratios.append(ratio)
        ...
        for i in range(1, 4, 1):
            k = 3 ** i
            ratio = sim_values[k - 3] / ch.calc_area_convex_hull_2d_pt2(1, k)
            weight = np.ones(len(ratio)) / len(ratio)
            ratios.append(ratio)
            weights.append(weight)
        ...
        for i in range(1, 4, 1):
            k = 3 ** i + 1
            ratio = sim_values[k - 4] / ch.calc_area_convex_hull_3d_pt2(1, 1, k)
            weight = np.ones(len(ratio)) / len(ratio)
            ratios.append(ratio)
            weights.append(weight)
        ax.set_xlabel(r"$SA_{ch}/E(SA_{ch})$", fontsize=21)
    elif measure == "volume":
        for i in range(1, 4, 1):
            k = 3 ** i + 1
            ratio = sim_values[k - 4] / ch.calc_volume_convex_hull_3d_pt2(1, 1, k)
            weight = np.ones(len(ratio)) / len(ratio)
            ratios.append(ratio)
            weights.append(weight)
        ax.set_xlabel(r"$V_{ch}/E(V_{ch})$", fontsize=21)
    ax.hist(x=ratios, bins=16, weights=weights, range=(0, 3))
    ax.set_xlim(0, 3)
    ax.set_ylim(0, ylim)
    ax.set_ylabel("Probability", fontsize=21)
    ax.tick_params(labelsize=20, width=2, length=6)
    tick_spacing_x = 1
    ax.xaxis.set_major_locator(ticker.MultipleLocator(tick_spacing_x))
    ax.yaxis.set_major_locator(ticker.MultipleLocator(tick_spacing_y))
...
def plot_3(measure, sim_values, n, ylim, tick_spacing_y):
    """
